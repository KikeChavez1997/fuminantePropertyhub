@extends('layouts.app', ['class' => 'off-canvas-sidebar', 'activePage' => '', 'title' => __('')])
      ...
      <form class="form" method="POST" action="{{ route('password.update') }}">
        ...
        <div class="card card-login card-hidden mb-3">
          ...
          <div class="card-body ">
            <div class="bmd-form-group{{ $errors->has('email') ? ' has-danger' : '' }} mt-3">
              <div class="input-group">
                ...
                <input type="email" name="email" class="form-control" placeholder="{{ __('Correo eletrónico') }}" value="{{ old('email') }}" required>
              </div>
              @if ($errors->has('email'))
          ...
          <div class="card-footer justify-content-center">
            <button type="submit" class="btn btn-info btn-link btn-lg">{{ __('Actualizar contraseña') }}</button>
          </div>
        </div>
      </form>
    </div>
